...
export default async function AgendaPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-10 py-10">
      ...
      {passados.length > 0 && (
        <div className="mt-14 pt-8 border-t border-gold-200/40 text-center">
          <Video className="w-5 h-5 text-gold-500 mx-auto mb-3" />
          <p className="text-sm text-ink-soft">
            Os encontros já realizados ficam em{" "}
            <a href="/jornada/gravacoes" className="text-gold-700 hover:underline">
              Gravações
            </a>
            .
          </p>
...
function CardProximo({ encontro }: { encontro: Encontro }) {
  ...
  return (
    <div className="rounded-3xl bg-gold-gradient/40 gold-border-rich gold-shadow-lg p-8 sm:p-10">
      <div className="flex items-center justify-between mb-6">
        <p className="text-[10px] tracking-[0.3em] uppercase text-gold-700">
          Próximo
        </p>
        <p className="text-[11px] tracking-[0.2em] uppercase text-gold-700">
          {proximo ? "Hoje" : dias === 0 ? "Hoje" : dias === 1 ? "Amanhã" : `Faltam ${dias} dias`}
        </p>
      </div>

      <h2 className="font-serif italic text-3xl sm:text-4xl gold-text-rich mb-4">
        {encontro.titulo}
      </h2>
      ...
      {encontro.link ? (
        <>
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-4 rounded-md bg-gold-foil text-white gold-shadow hover:opacity-90 text-sm tracking-[0.1em] uppercase font-medium"
          >
            <ExternalLink className="w-4 h-4" />
            Entrar na sala
          </a>
          <p className="text-[11px] text-gold-700 italic mt-3">
...
function LinhaEncontro({ encontro }: { encontro: Encontro }) {
  return (
    <div className="flex items-start gap-4 rounded-xl bg-cream-50 gold-border p-4">
      <div className="text-center shrink-0 w-14">
        <p className="text-[9px] tracking-[0.2em] uppercase text-gold-600">
          {new Date(encontro.data_inicio).toLocaleDateString("pt-PT", {
            month: "short",
          })}
        </p>
        <p className="font-serif italic text-2xl gold-text-rich leading-none">
      ...
      {encontro.link && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-xs tracking-[0.1em] uppercase text-gold-700 hover:text-gold-800 shrink-0 self-center"
        >
          Entrar →
        </a>
      )}
