      ...
      <main className="flex-1">
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <div className="mb-8">
            ...
            <h1 className="font-serif italic text-4xl sm:text-5xl gold-text-rich mb-2">
          ...
          {/* Progresso resumo */}
          <div className="rounded-2xl bg-cream-50 gold-border gold-shadow p-6 mb-8">
            <div className="grid gap-6 sm:grid-cols-3">
              <div>
                <p className="text-[10px] tracking-[0.3em] uppercase text-gold-600 mb-2">
                  Progresso
                </p>
                <p className="font-serif italic text-3xl gold-text-rich">
                  {aluna.conteudos_feitos}
                  <span className="text-ink-faint">/{aluna.conteudos_totais}</span>
                </p>
                <div className="h-1.5 rounded-full bg-gold-100 mt-3 overflow-hidden">
                  <div
          ...
          <div className="space-y-4">
            {aluna.estacoes.map((estacao) => {
              ...
              return (
                <div
                  key={estacao.id}
                  className="rounded-2xl bg-cream-50 gold-border overflow-hidden"
                >
                  <div className="flex items-center gap-3 px-5 py-4 border-b border-gold-200/40">
                    <span className="text-[11px] font-serif italic text-gold-600 w-8">
                      {String(estacao.numero).padStart(2, "0")}
                    </span>
                    <span className="font-serif italic text-lg gold-text-rich flex-1">
                      {estacao.nome}
                    </span>
                    {!estacao.publicada && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full uppercase tracking-[0.15em] bg-cream-100 text-ink-faint">
                        Rascunho
                      </span>
                    )}
                      ...
                      {estacao.conteudos.map((c) => {
                        ...
                        return (
                          <li
                            key={c.id}
                            className={cn(
                              "flex items-center gap-3 px-5 py-3 border-t border-gold-200/30 first:border-0",
                              feito ? "bg-success/5" : "",
                            )}
                          >
                            {feito ? (
