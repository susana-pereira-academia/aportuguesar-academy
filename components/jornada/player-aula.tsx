    ...
    <div className="max-w-4xl mx-auto px-6 lg:px-10 py-8 lg:py-12">
      ...
      <div className="mb-6">
        <p className="text-[10px] tracking-[0.3em] uppercase text-gold-600 mb-2">
          Estação {String(estacao.numero).padStart(2, "0")} · {estacao.nome}
        </p>
      ...
      {conteudo.tipo === "video" && conteudo.vimeo_id && (
        <div className="aspect-video rounded-2xl overflow-hidden bg-ink gold-shadow-lg mb-8">
          <iframe
      ...
      {conteudo.tipo === "material" && conteudo.material_url && (
        <div className="rounded-2xl bg-cream-50 gold-border-rich p-8 mb-8 text-center">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-md gold-border-rich text-gold-700 hover:bg-gold-50 text-sm tracking-[0.05em] uppercase font-medium transition-all"
          >
      ...
      {/* Descrição */}
      {conteudo.descricao && (
        <div className="rounded-2xl bg-cream-50/60 p-6 mb-8">
        ...
        <button
          ...
          className={cn(
            ...
            concluido
              ...
              : "bg-gold-foil text-white border-transparent hover:opacity-90 gold-shadow",
          )}
        >
          {concluido ? (
            <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 border-t border-gold-200/40 pt-6">
        {anterior ? (
          <Link
            href={anterior.href}
            className="group rounded-xl bg-cream-50 gold-border p-4 flex items-start gap-3 hover:gold-shadow transition-all"
          >
            <ChevronLeft className="w-4 h-4 text-gold-600 mt-1 shrink-0 group-hover:-translate-x-0.5 transition-transform" />
        ...
        {seguinte ? (
          <Link
            href={seguinte.href}
            className="group rounded-xl bg-cream-50 gold-border p-4 flex items-start gap-3 justify-end text-right hover:gold-shadow transition-all sm:col-start-2"
          >
            <div className="min-w-0">
              <p className="text-[10px] tracking-[0.3em] uppercase text-gold-600 mb-1">
                Seguinte
            ...
            <ChevronRight className="w-4 h-4 text-gold-600 mt-1 shrink-0 group-hover:translate-x-0.5 transition-transform" />
          </Link>
        ) : null}
      </div>
    </div>
